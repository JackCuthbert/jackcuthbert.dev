...
For the proxy we've made use of the package
[next-http-proxy-middleware](https://github.com/stegano/next-http-proxy-middleware)
which creates a Next.js handler function that can catch all the requests made to
`/api` and forward them to our proxy target configured by a
`process.env.API_HOST` variable.
